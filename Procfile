...
# - Bind the CMS dev server to ::1 explicitly (--host ::1) so it matches
#   HOST=::1 — which is what the routes file's `cms` upstream and the proxy's
#   CMS_UPSTREAM point at. (astro/vite ignores the HOST env var; the flag is
#   what actually binds it.) Sandboxed preview dev servers follow HOST too.
...
cms: SKIP_AUTH=true HOST=::1 bash scripts/launch-with-sandbox.sh pnpm dev -- --host '::1' | cat
