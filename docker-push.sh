#!/usr/bin/env bash
# Build the cms-agent OCI image (CMS + proxy in one container) with nix and
# copy it to the GitLab registry.
...
PROJECT="${CI_PROJECT_PATH:-plan-ai/cms-agent}"
TAG="${CI_COMMIT_SHORT_SHA:-latest}"

nix build ".#docker" -L
archive="$(readlink -f result)"

copy_image "$archive" "docker://${REGISTRY}/${PROJECT}/cms-agent:${TAG}"
copy_image "$archive" "docker://${REGISTRY}/${PROJECT}/cms-agent:latest"
