...
import { showFileAtRef } from '@/lib/git/engine';
...
const listConflictsTool: ToolDef = {
  ...
  async execute(_input, ctx) {
    ...
    const merging = fs.existsSync(`${ctx.worktreePath}/.git`)
      ? await git
    ...
    return JSON.stringify({
      ...
      note: status.conflicted.length
        ? 'Use show_conflict per file, resolve the markers (edit_file or resolve_conflict_take), then git_commit to conclude the merge.'
        : 'No conflicted files.',
    });
  },
};
