/**
 * Hex → human color names for chat rendering: `#7852ee` becomes a swatch +
 * "blueviolet" (nearest CSS keyword) with the raw hex in the tooltip.
 * ponytail: 140-keyword nearest-match beats shipping a color-naming dep.
 */

// The CSS named colors (level 4), name → rgb packed as 0xRRGGBB.
const CSS_COLORS: Record<string, number> = {
  ...
  oldlace: 0xfdf5e6, olivedrab: 0x6b8e23, orange: 0xffa500,
  ...
  plum: 0xdda0dd, powderblue: 0xb0e0e6, rebeccapurple: 0x663399,
  ...
  violet: 0xee82ee, wheat: 0xf5deb3, whitesmoke: 0xf5f5f5,
...
/** Parse #rgb / #rrggbb into packed RGB, null when malformed. */
...
/**
 * Name for a hex color: exact keyword, "~keyword" when perceptibly close,
 * null when nothing is near (caller keeps showing the hex).
 */
export const colorName = (hex: string): string | null => {
  const packed = parseHex(hex);
  if (packed === null) return null;
  const [r, g, b] = rgb(packed);
  let bestName = '';
  let bestDist = Infinity;
  for (const [name, value] of Object.entries(CSS_COLORS)) {
    const [nr, ng, nb] = rgb(value);
  ...
  return bestDist <= 3600 ? `~${bestName}` : null; // ≤60 per-channel-ish radius
};

const HEX_RE = /#(?:[0-9a-f]{6}|[0-9a-f]{3})\b/gi;

/** One hex occurrence → swatch + name (hex stays in the title tooltip). */
export const colorChipHtml = (hex: string): string => {
  const name = colorName(hex);
  const label = name ?? hex;
  return (
    `<span class="chat-color" title="${hex}">` +
    `<i class="chat-color__swatch" style="background:${hex}"></i>${label}</span>`
  );
};

/**
 * Replace hex colors in already-rendered HTML with swatch chips. Only text
 * between tags is touched — attribute values (hrefs, inline styles) and the
 * chips' own markup stay intact.
 */
export const annotateColorsInHtml = (html: string): string =>
  html
    .split(/(<[^>]*>)/)
    .map((part) => (part.startsWith('<') ? part : part.replace(HEX_RE, colorChipHtml)))
    .join('');
