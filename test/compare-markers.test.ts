...
describe("computeAnchors", () => {
  ...
  it("anchors a reworded section by its structure when the text changed", () => {
    ...
    const a = [
      mkc("SECTION/2/H1Pp", 100),
      mk("H1:Old heading#1", 110),
      mk("P:Old body#1", 160),
    ];
    const b = [
      mkc("SECTION/2/H1Pp", 300),
      mk("H1:New heading#1", 310),
      mk("P:New body#1", 360),
    ];
  ...
  it("matches reworded text by word overlap; unrelated text does not anchor", () => {
    const a = [
      mk("P:The useful gains of AI are real#1", 100),
      mk("P:Unrelated content here#1", 300),
    ];
    const b = [
      mk("P:The useful gains of AI are real and solid#1", 120),
      mk("P:Totally different words entirely#1", 350),
    ];
  ...
  it("never cross-matches different columns of the same flex/grid", () => {
    ...
    const b = [
      {
        k: "H4:The subscription ceiling#1",
        y: 0,
        sid: "grid",
        c: "card",
        fx: "G/3#2",
  ...
  it("skips a missing element (fill up) instead of mis-pairing the rest", () => {
    const a = [
      mk("P:Alpha block#1", 100),
      mk("P:Beta block#1", 200),
      mk("P:Gamma block#1", 300),
    ];
...
describe("matchConfidence", () => {
  const doc = (keys: string[], trunc = 0): MarkerDoc => ({
    h: 1000,
    trunc,
    m: keys.map((k, i) => ({ k: `${k}#1`, y: i * 20, i })),
  });

  it("identical content → high confidence", () => {
    const d = doc(["P:one", "P:two", "P:three", "H2:head"]);
    expect(matchConfidence(d, d).score).toBeGreaterThan(0.9);
  });

  it("total rewrite (nothing matches) → low confidence", () => {
  ...
  it("truncation clamps confidence even on a good match", () => {
    const keys = ["P:one", "P:two", "P:three", "H2:head"];
    const c = matchConfidence(doc(keys, 42), doc(keys));
    expect(c.truncated).toBe(true);
    expect(c.score).toBeLessThanOrEqual(0.6);
  });

  it("heavy duplicate/boilerplate text lowers confidence", () => {
    const clones = ["P:read more", "P:read more", "P:read more", "P:read more"];
    const c = matchConfidence(doc(clones), doc(clones));
    expect(c.dupPressure).toBeGreaterThan(0.5);
    expect(c.score).toBeLessThan(
      matchConfidence(
        doc(["P:a", "P:b", "P:c", "P:d"]),
        doc(["P:a", "P:b", "P:c", "P:d"]),
      ).score,
